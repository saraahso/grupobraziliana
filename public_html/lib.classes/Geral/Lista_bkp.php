...
class Lista{
	  ...
	  */
	  public function listar($ordem = "", $campo = '', $campoRand = 'id'){
		  ...
		  if($this->getTotal() > 0 && !$this->limiteDefinido){
			...
			if(empty($this->parametros['pos']))
				$this->parametros['pos'] = 0;
			if(empty($this->parametros['limite'])){
				$this->parametros['limite'] = 20;
			}
			  
			$this->limiteDefinido = true;
			
			$limit = " LIMIT ".$this->parametros['pos'].", ".$this->parametros['limite'];
			//echo $this->tabela.": ".$this->parametros['limite']."<br>";
			//echo $this->join.($this->group ? ' GROUP BY '.$this->group : '').$order.$limit."<br>";
	  ...
	  public function clearCache($pasta = ''){
		 ...
		 if(empty($pasta))
		 	$pasta = Sistema::$caminhoDiretorio."lib.data/cache/";
