(function() {
	...
	angular.module('app', ['Application', 'ui.router', 'ngRoute', 'ngAnimate']).config([
		...
		.run(['$rootScope', '$state', '$document', 'blockUI', '$window', '$location', '$interval', '$http', function($rootScope, $state, $document, blockUI, $window, $location, $interval, $http) {

			$rootScope.frontURL 						= document.location.origin + "/";
      $rootScope.backURL        	 	  = $rootScope.frontURL;
      //$rootScope.backURL        			= 'http://10.211.55.39/';
			//$rootScope.backURL          		= 'https://marktronic-jhonnybail.c9users.io/';
			$rootScope.backURL							= 'http://www.grupobraziliana.com.br/';
        ...
        $rootScope.marcas	= data.data.marcas;
				...
				$interval(function() {
					var revo = jQuery('.tp-banner').revolution(
					{
						delay: 5000,
						startwidth: 822,
						startheight: 535,
					...
					revo.bind("revolution.slide.onchange",function (e,data) {
            $('.slide').css('background-color', '#'+$rootScope.slides[data.slideIndex].corfundo);
					});
					//$('.slide').show();
					$(".link-submenu").click(function(ev){
				    var submenu = jQuery(ev.currentTarget).parent().find('.submenu');
				    submenu.slideToggle();
				  });
				}, 100, 1);
	...
	.controller('HomeController', ['$scope', '$rootScope', '$http', '$timeout', 'blockUI', function($scope, $rootScope, $http, $timeout, blockUI) {
		...
		if (!$rootScope.home) {
			...
			$scope.categorias = $rootScope.home.categorias;
			$timeout(function() {
				blockMain.stop();
				$("img.lazy").lazyload({
		            effect: "fadeIn"
	...
	.controller('CategoriaController', ['$scope', '$rootScope', '$http', '$state', '$timeout', '$interval', 'blockUI', function($scope, $rootScope, $http, $state, $timeout, $interval, blockUI) {
		...
		var cat = $state.params.idcategoria.split("-")[0],
				...
				blockMain = blockUI.instances.get('blockui-main');
		...
		function makeProduto() {
			if ($rootScope.produto.imagens) {
				$scope.imagens = {
					...
					lista: $rootScope.produto.imagens
				};
				delete $scope.imagens.lista.destaque;
				$timeout(function() {
					MagicZoom.refresh();
					blockMain.stop();
				}, 500);
	...
	.controller('VendedoresController', ['$scope', '$rootScope', '$http', '$timeout', 'blockUI', function($scope, $rootScope, $http, $timeout, blockUI) {
		...
		$('html, body').animate({
			scrollTop: ($(".sellers").offset().top - 70)
		}, 500);

		$http.get($rootScope.backURL + "api/vendedores").then(function(data) {
			$rootScope.sellers = data.data.data.result;
			$timeout(function() {
				$("img.lazy").lazyload({
					effect: "fadeIn"
				});
  ...
  .controller('MarcasController', ['$scope', '$rootScope', '$http', 'blockUI', '$timeout', function ($scope, $rootScope, $http, blockUI, $timeout){
    ...
    $('html, body').animate({
      scrollTop: ($(".brands-page").offset().top - 70)
    }, 500);
